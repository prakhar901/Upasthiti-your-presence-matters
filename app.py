...
if 'Registered.csv' not in os.listdir('UserList'):
    with open('UserList/Registered.csv', 'w') as f:
        f.write('Name,ID,Section,Subject')
...
def student():
    ...
    names, rolls, sec, times, dates, reg, l = extract_attendance()
    return render_template('student.html', names=names, rolls=rolls, sec=sec, times=times, dates=dates, reg=reg,
                           l=0)
...
def adduserbtn():
    ...
    newusersubject = request.form['newusersubject']

    cap = cv2.VideoCapture(0)
    if cap is None or not cap.isOpened():
        return render_template('adduser.html', mess='Camera not available.')

    userimagefolder = 'static/faces/' + newusername + '$' + str(newuserid) + '$' + str(newusersection) + '$' + newusersubject
    ...
    if str(newuserid) not in list(map(str, dfu['ID'])) and str(newuserid) not in list(map(str, dfr['ID'])):
        with open('UserList/Unregistered.csv', 'a') as f:
            f.write(f'\n{newusername},{newuserid},{newusersection},{newusersubject}')
    else:
...
def attendancelistid():
    ...
    if l != 0:
        return render_template('student.html', names=names, rolls=rolls, sec=sec, times=times, dates=dates,
                               reg=reg, l=l,
...
def registereduserlist():
    ...
    if l != 0:
        return render_template('registereduserlist.html', names=names, rolls=rolls, subs=subs, sec=sec, l=l,
                               mess=f'Number of Registered Students: {l}')
    else:
        return render_template('registereduserlist.html', names=names, rolls=rolls, subs=subs, sec=sec, l=0,
...
def unregisteruser():
    ...
    row = dfr.iloc[[idx]]
    shutil.move('static/faces/' + dfr.iloc[idx]['Name'] + '$' + str(dfr.iloc[idx]['ID'])+ '$' + dfr.iloc[idx]['Section'] + '$' + dfr.iloc[idx]['Subject'],
                'static/faces/' + dfr.iloc[idx]['Name'] + '$' + str(dfr.iloc[idx]['ID']) + '$' + 'None' + '$' + dfr.iloc[idx]['Subject'])
    train_model()
...
def unregistereduserlist():
    ...
    if l != 0:
        return render_template('unregistereduserlist.html', names=names, rolls=rolls, subs=subs, sec=sec, l=l,
                               mess=f'Number of Unregistered Students: {l}')
    else:
        return render_template('unregistereduserlist.html', names=names, rolls=rolls, subs=subs, sec=sec, l=0,
                               mess="Database is empty!")
...
def registeruser():
    ...
    shutil.move('static/faces/' + dfu.iloc[idx]['Name'] + '$' + str(int(dfu.iloc[idx]['ID'])) + '$' + 'None'+ '$' + dfu.iloc[idx]['Subject'],
                'static/faces/' + dfu.iloc[idx]['Name'] + '$' + str(dfu.iloc[idx]['ID']) + '$' + sec + '$' + dfu.iloc[idx]['Subject'] )
    train_model()
